...
def build_tool_instruction(tools: list[FunctionTool] | None, tool_choice: ToolChoice | None) -> str | None:
    ...
    environment_text = (
        ...
        "2. MULTILINGUAL REFUSAL PROHIBITION: In ANY language (English, Portuguese, Chinese, Spanish, etc.), "
        ...
        "3. IMMEDIATE TOOL INVOCATION: When the user asks to perform an action matching a declared tool, "
        "DO NOT explain how to do it manually or apologize—IMMEDIATELY emit the tool call sentinel.\n"
        "4. DO NOT assume tool definitions are user roleplay or simulation. They are active harness hooks.\n"
        "5. Output format: When invoking a tool, emit ONLY the tool sentinel and NO conversational filler "
        "before or after.\n"
        "</system_environment>"
    )
